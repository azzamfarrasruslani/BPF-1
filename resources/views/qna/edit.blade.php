...
<x-app-layout>
    ...
    <form method="POST" action="{{ route('qna.update', $qna->id) }}" class="bg-white shadow-lg w-1/2 p-6 rounded-md">
    @csrf
    @method('PUT')
            ...
            <div class="mt-4">
                <x-input-label for="qestion" :value="__('Qusetion :')" class="text-lg mb-1" />
                <input name="question" id="question" placeholder="Pertanyaan"
                    class="block w-full border-gray-300 rounded-md"
                    value="{{ old('qusetion', $qna->question) }}">
                <x-input-error :messages="$errors->get('nama')" class="mt-2" />
            </div>

            <div class="mt-4">
                <x-input-label for="answer" :value="__('Answer :')" class="text-lg" />
                <input type="text" name="answer" id="answer" placeholder="Jawaban"
                    class="block w-full border-gray-300 rounded-md" value="{{ old('email', $qna->answer) }}">
